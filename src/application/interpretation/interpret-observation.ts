...
function validateCandidate(value: unknown): RawCandidate {
  ...
  if (!boundedString(value.subject, 160)) throw new InterpretationValidationError();
  if (
    !boundedString(value.predicate, 64) ||
    !/^[a-z][a-z0-9_]*$/.test(value.predicate as string)
  ) throw new InterpretationValidationError();
  if (!boundedString(value.explanation, 500)) throw new InterpretationValidationError();
  if (
...
export function validateCandidateSet(value: unknown) {
  if (!isRecord(value) || !hasExactKeys(value, ["candidates", "schemaVersion"])) {
    throw new InterpretationValidationError();
  }
  if (value.schemaVersion !== interpretationSchemaVersion) throw new InterpretationValidationError();
  if (!Array.isArray(value.candidates) || value.candidates.length > interpretationCandidateLimit) {
    throw new InterpretationValidationError();
  }
...
function assertSupportedJobVersions(job: ClaimedInterpretationJob) {
  if (
    ![interpretationPromptVersionV1, interpretationPromptVersionV2].includes(job.promptVersion) ||
...
export function buildInterpretationInput(job: ClaimedInterpretationJob) {
  ...
  const evidence = [...job.evidence].sort((left, right) => left.ordinal - right.ordinal);
  if (evidence.length === 0 || evidence.length > interpretationEvidenceLimit) throw new InterpretationValidationError();
  if (
    evidence.some((fragment, index) => fragment.ordinal !== index || !boundedString(fragment.exactText, 8_000) || !boundedString(fragment.contentHash, 128)) ||
    evidence.reduce((total, fragment) => total + fragment.exactText.length, 0) > interpretationEvidenceCharacterLimit
  ) throw new InterpretationValidationError();
  ...
  return {
    ...
    providerInput: {
      evidence: evidence.map((fragment, index) => ({ exactText: fragment.exactText, reference: `evidence-${index}` })),
      promptVersion: job.promptVersion,
      schemaVersion: job.schemaVersion,
...
async function prepareCandidates(output: unknown, evidence: InterpretationEvidenceFragment[]) {
  const raw = validateCandidateSet(output);
  const fragmentByReference = new Map([...evidence].sort((left, right) => left.ordinal - right.ordinal).map((fragment, index) => [`evidence-${index}`, fragment.id]));
  const prepared: PersistableCandidate[] = [];
  const keys = new Set<string>();
  for (const candidate of raw) {
    const evidenceFragmentIds = candidate.evidenceReferences.map((reference) => {
    ...
    const logicalKey = await sha256Hex(canonicalJson({ evidenceFragmentIds: [...evidenceFragmentIds].sort(), payload }));
...
function failure(error: unknown): { code: InterpretationFailureCode; retryable: boolean } {
  if (error instanceof InterpretationValidationError) return { code: "validation_failed", retryable: false };
  if (error instanceof InterpretationProviderError) {
    return { code: error.code, retryable: ["provider_unavailable", "timeout"].includes(error.code) };
  }
...
export async function processNextInterpretationJob(input: {
  ...
  try {
    ...
    const output = await input.provider.interpret(prepared.providerInput, { signal: input.signal });
    const candidates = await prepareCandidates(output, job.evidence);
    await input.repository.complete({ candidates, job, runId });
    return { candidateCount: candidates.length, jobId: job.id, state: "succeeded" as const };
  } catch (error) {
