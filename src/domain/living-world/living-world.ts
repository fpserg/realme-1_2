...
export function composeLivingWorld(
  ...
  const realmIds = nodes.filter((node) => isRealm(node.classification)).map((node) => node.id);
  ...
  for (const [depth, level] of [...levels.entries()].sort(([left], [right]) => left - right)) {
  ...
  projectedNodes.sort((left, right) => compareText(left.canonicalId, right.canonicalId));
  ...
  const structuralValue = JSON.stringify({
    edges,
    nodes: projectedNodes.map(({ canonicalId, classification, depth, label, x, y }) => ({
      canonicalId,
      classification,
      depth,
      label,
