...
export const jobs = pgTable(
  ...
  (table) => [
    uniqueIndex("jobs_world_kind_idempotency_unique").on(table.worldId, table.jobKind, table.idempotencyKey),
    unique("jobs_world_id_id_unique").on(table.worldId, table.id),
    unique("jobs_world_id_observation_id_unique").on(table.worldId, table.id, table.observationId),
    ...
    foreignKey({ name: "jobs_observation_world_fk", columns: [table.worldId, table.observationId], foreignColumns: [observations.worldId, observations.id] }).onDelete("restrict"),
    check("jobs_kind_not_blank", sql`length(btrim(${table.jobKind})) > 0`),
    check("jobs_idempotency_key_not_blank", sql`length(btrim(${table.idempotencyKey})) > 0`),
    check("jobs_status_check", sql`${table.status} in ('queued', 'running', 'succeeded', 'failed', 'cancelled')`),
    check("jobs_step_102_kind_check", sql`${table.jobKind} = 'interpret_observation'`),
    check("jobs_attempts_check", sql`${table.attempts} >= 0`),
    check("jobs_max_attempts_check", sql`${table.maxAttempts} > 0`),
    check("jobs_attempts_within_max_check", sql`${table.attempts} <= ${table.maxAttempts}`),
    check("jobs_queued_state_check", sql`${table.status} <> 'queued' or (${table.attempts} < ${table.maxAttempts} and ${table.lockedAt} is null and ${table.lockToken} is null)`),
    check("jobs_running_state_check", sql`${table.status} <> 'running' or (${table.lockedAt} is not null and ${table.lockToken} is not null and ${table.attempts} >= 1)`),
    check("jobs_non_running_unlocked_check", sql`${table.status} = 'running' or (${table.lockedAt} is null and ${table.lockToken} is null)`),
    ...
    check("jobs_last_failure_code_check", sql`${table.lastFailureCode} is null or ${table.lastFailureCode} in ('provider_unavailable', 'timeout', 'malformed_output', 'validation_failed', 'persistence_failed', 'configuration_error', 'cancelled', 'exhausted')`),
    check("jobs_success_has_no_failure_check", sql`${table.status} <> 'succeeded' or ${table.lastFailureCode} is null`),
  ],
);
...
export const auditEvents = pgTable(
  ...
  (table) => [
    index("audit_events_world_occurred_at_index").on(table.worldId, table.occurredAt),
    foreignKey({ name: "audit_events_actor_world_membership_fk", columns: [table.worldId, table.actorAccountId], foreignColumns: [worldMemberships.worldId, worldMemberships.userId] }).onDelete("restrict"),
    check("audit_events_actor_kind_check", sql`${table.actorKind} in ('user', 'system', 'policy')`),
    check("audit_events_user_actor_check", sql`${table.actorKind} <> 'user' or ${table.actorAccountId} is not null`),
    check("audit_events_action_not_blank", sql`length(btrim(${table.action})) > 0`),
    check("audit_events_entity_type_not_blank", sql`length(btrim(${table.entityType})) > 0`),
