ALTER TABLE "jobs" DROP CONSTRAINT "jobs_interpret_observation_input_check";--> statement-breakpoint
ALTER TABLE "jobs" ADD CONSTRAINT "jobs_interpret_observation_input_check" CHECK ("jobs"."job_kind" <> 'interpret_observation' or ("jobs"."observation_id" is not null and "jobs"."payload" ?& array['prompt_version', 'schema_version'] and ("jobs"."payload" - array['prompt_version', 'schema_version']) = '{}'::jsonb and jsonb_typeof("jobs"."payload"->'prompt_version') = 'string' and jsonb_typeof("jobs"."payload"->'schema_version') = 'string' and "jobs"."payload"->>'prompt_version' in ('interpret-observation-v1', 'interpret-observation-v2', 'interpret-observation-v3') and "jobs"."payload"->>'schema_version' = 'candidate-set-v1'));
...
BEGIN
  ...
  INSERT INTO public.jobs (
    ...
    'queued',
    jsonb_build_object(
      'prompt_version', 'interpret-observation-v3',
      'schema_version', 'candidate-set-v1'
    )
  )
    ...
    WHERE observation.recorded_by_account_id = v_actor_id
      ...
      AND NOT EXISTS (
        ...
        WHERE active_job.world_id = observation.world_id
          ...
          AND active_job.payload->>'prompt_version' = 'interpret-observation-v3'
