...
function requireIsoInstant(value, itemId) {
  ...
  if (Number.isNaN(parsed.getTime()) || parsed.toISOString() !== value) {
    throw new SourceValidationError(
      `${itemId}: occurredAt must be an exact ISO instant or null`,
    );
  }
  return value;
}
...
export function validateManifest(manifest) {
  if (!manifest || manifest.version !== 1) {
    throw new SourceValidationError("manifest version must be 1");
  }
  if (
    !manifest.sourceRepository ||
    !/^[^/]+\/[^/]+$/.test(manifest.sourceRepository)
  ) {
    throw new SourceValidationError("sourceRepository must be owner/repo");
  }
  if (!/^[0-9a-f]{40}$/.test(manifest.sourceCommit ?? "")) {
    throw new SourceValidationError("sourceCommit must be pinned");
  }
  if (!/^[0-9a-f]{40}$/.test(manifest.sourceTree ?? "")) {
    throw new SourceValidationError("sourceTree must be pinned");
  }
  if (!Array.isArray(manifest.items) || manifest.items.length === 0) {
    throw new SourceValidationError("items required");
  }

  const ids = new Set();
  for (const item of manifest.items) {
    if (!item.id || ids.has(item.id)) {
      throw new SourceValidationError("item ids must be unique");
    }
    ids.add(item.id);
    if (!AUTHORITY_CLASSES.has(item.authorityClass)) {
      throw new SourceValidationError(`${item.id}: invalid authority class`);
    }
    if (!/^[0-9a-f]{40}$/.test(item.blobSha ?? "")) {
      throw new SourceValidationError(`${item.id}: blobSha required`);
    }
    if (!item.path || item.path.includes("..")) {
      throw new SourceValidationError(`${item.id}: unsafe path`);
    }
    if (!["import", "exclude"].includes(item.action)) {
      throw new SourceValidationError(`${item.id}: invalid action`);
    }
    if (item.action === "exclude" && !item.excludeReason) {
      throw new SourceValidationError(`${item.id}: exclude reason required`);
    }
    if (
      item.action === "import" &&
      !IMPORTABLE_CLASSES.has(item.authorityClass)
    ) {
      throw new SourceValidationError(
        `${item.id}: presentation-only material cannot be imported`,
      );
    }
    if (
      !item.selection ||
      !["whole_file", "exact_text"].includes(item.selection.kind)
    ) {
      throw new SourceValidationError(`${item.id}: unsupported selection`);
    }
    if (item.selection.kind === "exact_text" && !item.selection.text) {
      throw new SourceValidationError(
        `${item.id}: exact_text selection requires text`,
      );
    }
...
export function extractSourceText(fileText, item) {
  if (typeof fileText !== "string") {
    throw new SourceValidationError(`${item.id}: source file missing`);
  }
  ...
  if (first < 0) {
    throw new SourceValidationError(`${item.id}: exact text not found`);
  }
...
export function buildSourceLocator(manifest, item) {
  const selection =
    item.selection.kind === "whole_file"
      ? "whole_file"
      : `exact_text:${sha256(item.selection.text)}`;
...
export function assertNoCanonicalWrites(payload) {
  for (const key of Object.keys(payload ?? {})) {
    if (CANONICAL_TABLES.has(key)) {
      throw new SourceValidationError(
        `import planner attempted canonical write: ${key}`,
      );
    }
  }
  return true;
...
export function resolveExplicitIdentityMappings(entries) {
  ...
  for (const entry of entries) {
    if (!entry.sourceIdentity || !entry.nativeNodeId || !entry.label) {
      throw new SourceValidationError("identity mapping is incomplete");
    }

    const prior = bySource.get(entry.sourceIdentity);
    if (prior && prior !== entry.nativeNodeId) {
      throw new AmbiguousIdentityError(
        `source identity ${entry.sourceIdentity} maps to multiple native ids`,
      );
    }
