...
describe("Step 107 control-plane artifact", () => {
  it("keeps the approved manifest at five imports, one Class E exclusion and unknown instants", () => {
    const included = approvedManifest.items.filter((item) => item.action === "import");
    const excluded = approvedManifest.items.filter((item) => item.action === "exclude");
  ...
  it("builds deterministic byte-identical SQL artifacts", async () => {
    const template = await readFile(
      resolve(process.cwd(), "scripts/step-107-import-evidence-control-plane.sql"),
      "utf8",
    );
    ...
    expect(() =>
      buildControlPlanePayload(
        approvedManifest,
        {
          ...plan,
          included: [
            { ...plan.included[0], occurredAt: "2026-08-30T10:15:00.000Z" },
            ...plan.included.slice(1),
          ],
        },
        worldId,
  ...
  it("contains fail-closed ownership, reconciliation, rollback failpoint and canonical guards", async () => {
    ...
    for (const table of ["admission_decisions", "ontology_nodes", "assertions"]) {
