...
describe("Step 105 Living World boundary", () => {
  it("reads only admitted World Model structure and exposes no mutation path", async () => {
    ...
    expect(source).not.toMatch(/candidate_claims|interpretation_runs|source_fragments/);
  ...
  it("keeps the renderer code-native and explicitly versioned", async () => {
    const source = await readFile("src/domain/living-world/living-world.ts", "utf8");

    expect(source).toContain('LIVING_WORLD_RENDERER_VERSION = "living-world-code-v1"');
