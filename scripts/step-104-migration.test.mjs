...
describe("Step 104 commitment projections", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(sql).not.toMatch(
        /UPDATE public\.(observations|candidate_claims|admission_decisions|ontology_nodes|ontology_aliases|ontology_relationships|assertions|assertion_evidence)/i,
      );
      expect(sql).not.toMatch(
        /DELETE FROM public\.(observations|candidate_claims|admission_decisions|ontology_nodes|ontology_aliases|ontology_relationships|assertions|assertion_evidence)/i,
      );
    },
  );

  it(
    "derives World, civil operational date, Today, Horizon and stale state server-side",
    async () => {
      const sql = await readFile(migrationPath, "utf8");
      const fn = sql.slice(
        sql.indexOf(
  ...
  it(
    "carries an explicit rollback-only destroy/rebuild equivalence proof",
    async () => {
